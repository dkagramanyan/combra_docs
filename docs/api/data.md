...
````{py:function} combra.data.plot_poliamid_fractal(parquet, step=0.01, ncols=3, xlim=(1.0, 2.0), ylim=(0.0, 0.05), save=None, show=True) -> plotly.graph_objects.Figure

Grid of per-group contour fractal-dimension distributions. For each frame group in a {py:meth}`~combra.data.PoliamidDataset.generate` parquet, the `fd_list` is binned with {py:func}`~combra.stats.stats_preprocess` and fitted with {py:func}`~combra.approx.bimodal_gauss_approx`; the two fitted mode means/widths appear in each subplot title.

:param parquet: Parquet produced by {py:meth}`~combra.data.PoliamidDataset.generate`.
...
```python
>>> from combra import data
>>> data.plot_poliamid_fractal('poliamid_group250_n1000.parquet', step=0.01)
```
````

````{py:function} combra.data.plot_poliamid_contour(parquet, metric='area', dists=('gauss',), step=1.0, ncols=4, x_lim=(0.0, 25.0), grid_n=100, save=None, show=True) -> plotly.graph_objects.Figure

Grid of per-group contour-size distributions with overlaid distribution fits. For each frame group the chosen `metric` column is binned with {py:func}`~combra.stats.stats_preprocess` and one fitted curve per entry in `dists` is overlaid (using {py:func}`~combra.approx.binomial_approx` / {py:func}`~combra.approx.poisson_approx` / {py:func}`~combra.approx.gauss_approx` / {py:func}`~combra.approx.exponential_approx`).
